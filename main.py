...
def nifty_loop():
    ...
    while True:
        try:
            ...
            # ── DEBUG: last 6 bars of HalfTrend with timestamps ─────────────
            _dbg = ht_df.iloc[-7:-1][["trend","buy","sell","ht"]].copy()
            print(f"   📊 Last bar timestamp: {ht_df.index[-1]}  |  iloc[-2]: {ht_df.index[-2]}", flush=True)
            for _idx, _row in _dbg.iterrows():
                _arr = "🟢BUY" if _row["buy"] else ("🔴SELL" if _row["sell"] else "·")
                _ts = str(_idx).replace("+05:30","").strip()[-14:]
                print(f"   {_ts}  trend={'CALL' if _row['trend']==0 else 'PUT'}  {_arr}  ht={_row['ht']:.1f}", flush=True)
